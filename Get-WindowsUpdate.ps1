function Install-WindowsUpdate {
    ...
    begin {
        ...
        $PSCmdlet.WriteVerbose("[$($searchResults.Updates.Count)] updates are available.")
        ...
        foreach ($update in $searchResults.Updates) {
            if ((-not $update.EulaAccepted) {
                if (-not $AcceptEula) {
                    $PSCmdlet.WriteWarning("The update [$($update.Title)] requires the EULA to be accpeted to install it. To accept the EULA use the [-AcceptEula] parameter.")
                    continue
                }
                $PSCmdlet.WriteWarning("Accepting the EULA for the update [$($update.Title)] with the following terms and conditions: rn$($update.EulaText)")
                $update.AcceptEula()
            }
        ...
        $PSCmdlet.WriteVerbose("Downloading [$($updateColl.Updates.Count)] updates.")
        ...
        $PSCmdlet.WriteVerbose("Installing [$($updateColl.Updates.Count)] updates.")
